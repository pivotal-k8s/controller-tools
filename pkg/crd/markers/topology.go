...
// list is an associative-list or a set, if a map is atomic or not).
...
//
// ListMapKey indicates the index of a map list. They can be repeated if
// multiple keys must be used. It can only be used when ListType is set
// to map, and the keys should be scalar types.
...
func (l ListMapKey) ApplyToSchema(schema *v1beta1.JSONSchemaProps) error {
	if schema.Type != "array" {
...
func (m MapType) ApplyToSchema(schema *apiext.JSONSchemaProps) error {
	// if schema.Type != "map" {
	// 	//TODO error; this only applies to maps
	// }
	//
	// if m != "one of atomic or granular" {
	// 	//TODO error; only atomic/granular are valid options
	// }
	//
	// //otherwise apply to schema
	// p := string(m)
	// schema.XMapType = &p
